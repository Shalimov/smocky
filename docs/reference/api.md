...
Smocky ships as both a runnable CLI and an importable library. This page
is the canonical reference for both.

## CLI

The package's `bin` field exposes the `smocky` command. After installing
with `bun add -d github:YOUR_USER/smocky#v0.1.0`, run it with `bun
smocky <command>`. When developing inside this repo, use `bun run
src/cli/index.ts <command>` instead.

```
smocky <command> [options]
...
init options:
  ...
  --port <n>                 Port baked into smocky.config.ts (default 3000)
...
without a subcommand, `all` is assumed. See [`smocky init`](../features/init.md)
...
smocky init --from-openapi ./openapi.yaml --yes

# Start the server with defaults
smocky

# Use a different config and port
smocky --config ./mocks/smocky.config.ts --port 4000
...
smocky check api --base-url https://staging.example.com
```

## Library

Smocky is importable from the `smocky` package:
...
const server = await startServer({
  config: './smocky.config.ts',   // optional; defaults to cwd lookup
...
### Spin Smocky up inside a test runner

```ts
import { startServer, type ServerHandle } from 'smocky';
...
Smocky follows semver. The CLI surface and the exports listed above
